...
我是**四件式工具鏈**的中段，搭 [Cartographer](https://github.com/RayLi-Git/cartographer)（生 PRD）、[Sentinel](https://github.com/RayLi-Git/sentinel)（怎麼想）與 [Lookout](https://github.com/RayLi-Git/lookout)（段落完成的獨立審查）：**Cartographer 畫地圖 → Compass 照圖走 → Sentinel 站哨 → Lookout 瞭望**。Sentinel 看「怎麼想」（淺層 vs 深層、根因 vs 症狀），Compass 看「怎麼照規格執行」，Lookout 在段落完成後開獨立 context 補審，常一起用。
...
⚠️ **本 skill 涵蓋什麼 / 不涵蓋什麼 → 載 [`references/00_scope/`](./references/00_scope/_index.md)**（完整設計取捨與未來方向見 GitHub repo 的 [`docs/SCOPE.zh-TW.md`](https://github.com/RayLi-Git/compass/blob/main/docs/SCOPE.zh-TW.md)）

---

## 🗺️ 主題模組（按需載入對應的 references/）

> §0 範圍邊界 + §1–§11 全模組 + §6.6 SLA + §11 可執行腳本 + 中英雙語，皆已交付。完整設計取捨與未來方向見 GitHub repo 的 [`docs/SCOPE.zh-TW.md`](https://github.com/RayLi-Git/compass/blob/main/docs/SCOPE.zh-TW.md)。

### §0 Scope — 開工前先確認範圍
- 載：`references/00_scope/`
- 何時：不確定這個任務該不該用 Compass（vs 用 Sentinel 就好）時
...
- [`references/00_scope/`](./references/00_scope/_index.md) — 本 skill 涵蓋什麼 / 不涵蓋什麼（**不確定要不要用 Compass 時先讀**）

以下為 GitHub repo 文件（依官方安裝方式不會出現在本地 skill 目錄，連結指向 GitHub）：
- [docs/SCOPE.zh-TW.md](https://github.com/RayLi-Git/compass/blob/main/docs/SCOPE.zh-TW.md) — 完整範圍、設計取捨與未來方向
- [docs/DESIGN.zh-TW.md](https://github.com/RayLi-Git/compass/blob/main/docs/DESIGN.zh-TW.md) — 設計決策與取捨
- [docs/INSTALL.zh-TW.md](https://github.com/RayLi-Git/compass/blob/main/docs/INSTALL.zh-TW.md) — 安裝指南
- [README.zh-TW.md](https://github.com/RayLi-Git/compass/blob/main/README.zh-TW.md) · [README.md](https://github.com/RayLi-Git/compass/blob/main/README.md) — 專案總覽（中 / 英）
