...
我跟 [Sentinel](https://github.com/RayLi-Git/sentinel) 是**配對 skill**——Sentinel 看「怎麼想」（淺層 vs 深層、根因 vs 症狀），Compass 看「怎麼照規格執行」。兩個常一起用。
...
> §1–§11 全模組已有內容（v0.9.0 / Phase 1-4 交付，含 §6.6 SLA + §11 可執行腳本）。剩餘：英文化、INSTALL/DESIGN 文件（Phase 5）。詳見 [docs/SCOPE.zh-TW.md](./docs/SCOPE.zh-TW.md) 的開發路線圖。
...
Compass 跟 Sentinel 共用 `.claude/` 病歷檔。當 Compass 遇到的「PRD bug / 模糊處置 / 設計取捨」夠痛時，也寫進 `.claude/debug-log.md`，但加標 `[COMPASS]` 前綴方便檢索。
...
- docs/DESIGN.zh-TW.md — 設計決策與取捨（🚧 Phase 5 撰寫中）
- [docs/INSTALL.zh-TW.md](./docs/INSTALL.zh-TW.md) — 安裝指南
- [README.zh-TW.md](./README.zh-TW.md) — 專案總覽（英文版 README.md 🚧 Phase 5）

---

**Version**: v0.9.0 (Phase 1-4 完成 — §1–§11 全模組 + 可執行腳本 + 模板)
**Status**: 🚧 WIP — 英文化、INSTALL/DESIGN 文件（Phase 5）待補
